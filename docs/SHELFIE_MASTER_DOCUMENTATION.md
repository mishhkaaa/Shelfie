...
- **In-app apply no longer guesses `f=` facet keys.** `applyWithRetry` (`inpage/retryApply.ts`) now navigates to a bare `category + price` URL (the only two facets confirmed to serialize the same way everywhere) and applies every other facet — fabric, sleeve, neck, size, color, occasion, brand, gender, `other` — by finding and clicking the real, already-rendered filter checkbox on the page (`inpage/applyFiltersViaDom.ts`), so it can never send Myntra a facet key it doesn't recognize. The old zero-result drop-and-retry loop still runs on top of this, now recovering from genuinely-empty filter *combinations* rather than guessed-key failures.
  - **Not yet live-tested.** The checkbox-matching logic (leaf elements matching `"Label (count)"`, e.g. `"Cotton (233)"`) is inferred from screenshots, not confirmed against Myntra's real DOM/class names — expect one round of adjustment once tried against the live site.
