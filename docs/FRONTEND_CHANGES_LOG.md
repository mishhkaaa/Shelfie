...
## 2026-07-23 (Part 3, Tier 0) — CRITICAL FIX: "Apply to search" now actually navigates the real Myntra tab
**File(s):** extension/src/panel/IntentCompiler.tsx, extension/src/panel/ProfileList.tsx, extension/src/adapter/navigate.ts (new)
**What changed:** Real bug, confirmed by direct user testing: clicking "Apply to search" after the NL compiler only called `loadLiveConstraints` with the merged patch — this updated the side panel's *internal* state only. The actual Myntra tab never re-navigated, so the page stayed unfiltered while the panel showed something different, and a profile saved right after would not match what was genuinely on screen. Fixed by extracting the exact navigation mechanism `ProfileList.handleActivate` already used (build the URL via `buildUrlFromConstraints`, send `NAVIGATE_TO` to the content script, hard-navigate via `chrome.tabs.update` as a fallback) into a single shared `navigateActiveTabTo()` helper, used by both `ProfileList` (reopening a saved profile) and now `IntentCompiler` (applying a compiled patch) — no second parallel implementation. `IntentCompiler` no longer calls `loadLiveConstraints` directly at all: `liveConstraints` only ever gets set by the real content-script pipeline re-parsing the URL after navigation, exactly like every other filter change. This also resolves the second concern raised (a stray real URL-change event silently clobbering the just-applied patch) — there's no longer a competing write to guard against, since the compiler doesn't write to that state itself anymore.
Added a guard: if the merged constraints have no `category.articleType` (e.g. the sentence named a color/size but no product type, and there's no current live constraints to inherit one from), applying shows a visible error instead of navigating to an invalid `myntra.com/` path. Also noted inline that any `price.min`/`price.max` found by the compiler is not yet applied to the live page (the URL builder doesn't serialize price — see BACKEND_BUILD_LOG.md decision) rather than letting it silently vanish after navigation with no explanation.
**Why:** Direct, explicit user bug report ("this is something which should be fixed immediately").
**Action needed from teammate:** none — but note reopening a saved profile and applying a compiled patch now go through literally the same function, so a fix/regression in one affects both (intentional).

## 2026-07-23 (Part 3, Tier 0, light follow-up) — Compiler extraction: size abbreviations + more category phrasing
**File(s):** backend/app/ai/compile_intent.py
**What changed:** User also reported the sentence "birthday dresses for women black XS" wasn't extracting everything. Root cause: the size lexicon only had spelled-out words ("extra small") as keys, but sizes are commonly stated as bare abbreviations ("XS") — added the abbreviations themselves as additional lexicon keys (xs/s/m/l/xl, xxl already existed). Also added a few more `articleType` phrasings ("birthday dresses", "birthday dresses for women", "dresses for women", "women's dresses") since exact-match-only lexicon lookup is brittle to phrasing variance. Nudged the system prompt to explicitly ask for every distinct attribute as a separate proposal and to propose sizes as-written. This was flagged by the user as lower-priority/riskier to touch, so kept intentionally minimal — additive lexicon entries only, no change to the validation architecture.
**Why:** Direct user bug report, addressed at reduced scope per the user's own risk-aversion note.
**Action needed from teammate:** none. Verified: the exact reported sentence now extracts category, color, and size correctly; the existing "reject nonsense" regression test still passes unchanged.

## 2026-07-23 — Teammate merge: new in-page "Shelfie Discover" panel (discover/fuzzy branch, PR #2)
**File(s):** extension/src/inpage/* (new: InpageApp.tsx, DiscoverPanel.tsx, CombinedSearch.tsx, ForkControl.tsx, FloatingButton.tsx, SlideOver.tsx, mount.ts, retryApply.ts, store/useInpageStore.ts, inpage.css), extension/src/inpage-panel.ts (new), extension/src/main-world-interceptor.ts (new), extension/src/content-script.ts, extension/src/store/discoverActions.ts (new), extension/src/adapter/similarity.ts (new), extension/src/App.tsx, extension/src/store/useShelfieStore.ts, extension/manifest.json, extension/package.json, extension/vite.config.ts, extension/vite.content-scripts.config.ts (new), extension/src/panel/Discover.tsx (removed), extension/src/panel/IntentCompiler.tsx (removed)
**What changed:** Not authored by me — merged in from a teammate's branch, audited afterward rather than assumed. A whole second UI surface now exists: a floating button + popover injected directly onto the Myntra page itself (inside a Shadow DOM, isolated from Myntra's own CSS), backed by its own Zustand store (`useInpageStore` — a separate instance from the side panel's `useShelfieStore`; the two bundles share no runtime state at all, only pure-logic modules like `mergeConstraints`/`navigate`). A new MAIN-world script (`main-world-interceptor.ts` — the only way to see Myntra's own `fetch`/XHR calls, since an isolated-world content script has a completely separate `window` object) reports Myntra's real search-gateway result counts back to the extension; `inpage/retryApply.ts` uses this (plus a DOM-based "no results" check as a fallback for requests that 404 before ever reaching the gateway) to automatically drop the least-confident filter and retry, up to 5 attempts, whenever an applied filter combination returns nothing — instead of silently leaving the user on a dead page. The old side-panel `Discover.tsx`/`IntentCompiler.tsx` were removed in favor of this new in-page equivalent (`DiscoverPanel.tsx`/`CombinedSearch.tsx`, which also adds a fast client-side stemmed-word ranking shown immediately, replaced by the new `/discover/search` Groq semantic ranking once it resolves). `useShelfieStore.ts` gained a `refreshProfilesForPersona` action and `App.tsx` a `SHELFIE_PROFILE_FORKED` message listener, so a fork made in the (separate-store) in-page panel still shows up in the side panel immediately instead of only after switching personas away and back. `manifest.json` now declares three content scripts (the original isolated-world content-script, the new inpage-panel, and the new MAIN-world interceptor) plus a `web_accessible_resources` entry for the panel's compiled CSS; `package.json`'s build script now runs four extra single-entry Rollup builds (`vite.content-scripts.config.ts`) after the main `vite build`, since Chrome content scripts must be classic non-module scripts, unlike the side panel's real ES-module page.
**Why:** Not part of any master prompt given to me — a parallel feature direction built by the rest of the team.
**Action needed from teammate:** none from my side — flagged to the user that my own earlier `AgentBuilder` (a step-by-step agentic profile builder in the side panel) was lost in the process of this merge landing (it was never committed, and got discarded by a git operation) — this new in-page panel is the current, working replacement for natural-language search, not an addition alongside the lost one.

## 2026-07-23 — WhatsApp sharing: new ShareButton, wired into both the side panel and the in-page Discover popover
**File(s):** extension/src/components/ShareButton.tsx (new), extension/src/panel/ProfileList.tsx, extension/src/inpage/DiscoverPanel.tsx, extension/src/api/client.ts, extension/src/api/types.ts
**What changed:** New `src/components/` directory — the first React component shared directly between the side-panel bundle and the in-page bundle (everything shared before this was pure-logic adapters like `mergeConstraints`/`navigate`). `ShareButton` takes `{profileId, name, constraints}` as plain props and needs no store access at all (the side panel and in-page bundle use two entirely separate Zustand store instances, so a shared component can only work if it doesn't reach into either one) — it builds the Myntra link itself via the existing `buildUrlFromConstraints` and calls the new `POST /profiles/{id}/share` endpoint directly. Wired into `ProfileList.tsx` (shown only on your own profiles once toggled Public — restructured that card's layout from a single flex row to a row-plus-conditional-second-row to fit the button and its expandable phone-number form) and `inpage/DiscoverPanel.tsx` (shown on every public profile, alongside the existing Fork control).
**Why:** Direct user request — "I want the public profiles to be shareable... when I create them as well as from discover."
**Action needed from teammate:** none. One real WhatsApp send was tested end-to-end (with the user's explicit confirmation before sending, since it's a real-world side effect) and Twilio accepted it.
